...
function InfoItem({ label, value }: { label: string; value?: string | number | null }) {
  ...
  return (
    <div className="detail-info-item">
      ...
      <span className={`detail-info-value ${isEmpty ? "text-muted" : ""}`}>
        {isEmpty ? "—" : value}
      </span>
    </div>
  );
    ...
    <div className="page-stack">
      ...
      <section className="hero-card detail-hero">
        <div className="hero-copy">
          ...
          <div className="inline-tags" style={{ marginTop: 4 }}>
        ...
        <div className="hero-actions">
          <div className="detail-action-group">
            ...
            <button className="ghost-button" onClick={() => stopMutation.mutate()} disabled={stopMutation.isPending || !isRunning}>
              停止
            </button>
            <button className="ghost-button" onClick={() => rebootMutation.mutate()} disabled={rebootMutation.isPending || !isRunning}>
              重启
            </button>
          </div>
          <div className="detail-action-divider" />
          <div className="detail-action-group">
            <Link className="primary-button" to={`/terminal/${detail.instanceId}`}>
              SSH 终端
            </Link>
      ...
      <section className="detail-grid">
        <SectionCard title="硬件与系统" description="设备的硬件规格与底层系统配置">
          <div className="detail-info-table">
            <InfoItem label="实例 ID" value={detail.instanceId} />
            <InfoItem label="板卡型号" value={detail.boardType} />
            {detail.boardName ? <InfoItem label="板卡名称" value={detail.boardName} /> : null}
            <InfoItem label="内存" value={detail.memory ? `${detail.memory} GB` : undefined} />
            <InfoItem label="系统盘" value={detail.diskSize !== undefined ? `${detail.diskSize} GB` : undefined} />
            {detail.imageName ? <InfoItem label="所属镜像" value={detail.imageName} /> : null}
            {detail.osName ? <InfoItem label="操作系统" value={detail.osName} /> : null}
          </div>
        </SectionCard>

        <SectionCard title="网络与计费" description="实例的连接属性和账单信息">
          <div className="detail-info-table">
            <InfoItem label="内网 IP" value={detail.ipAddress} />
            {detail.regionName ? <InfoItem label="地域节点" value={detail.regionName} /> : null}
            <InfoItem label="计费模式" value={detail.billingMode} />
            {detail.monthlyPrice ? <InfoItem label="月预估费用" value={formatMoney(detail.monthlyPrice)} /> : null}
            {detail.bandwidth !== undefined ? (
              <InfoItem label="公网带宽" value={detail.bandwidthType ? `${detail.bandwidth} ${detail.bandwidthType}` : `${detail.bandwidth}`} />
            ) : null}
            <InfoItem label="创建时间" value={formatDateTime(detail.createdAt)} />
            {detail.expireAt ? <InfoItem label="到期时间" value={formatDateTime(detail.expireAt)} /> : null}
          </div>
        </SectionCard>
      </section>
